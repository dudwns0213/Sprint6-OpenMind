...
import AnswerButtonTop from "./components/AnswerButtonTop";
import DropdownMenu from "./components/DropdownMenu";
import UserCard from "./components/UserCard";
import { useEffect, useState } from "react";
import { getQuestionCard } from "@/api/OpenMindApi";
...
function QuestionListPage() {
  ...
  const getPageRange = (currentPage, totalPages, pageRange = 5) => {
    ...
    if (start < 1) {
      // currentPage가 1,2,3 일 경우에 1,2,3,4,5 페이지를 유지하는 조건문
      start = 1;
      end = pageRange;
    }

    if (end > totalPages) {
      // currentPage가 끝쪽에 도달했을때 end 변수값이 전체 페이지값 보다 높을시 조건실행
  ...
  useEffect(() => {
    const handleCardItemLoad = async () => {
      let result = await getQuestionCard({ sort, offset, limit });
  ...
  return (
    <Container>
      ...
      <PaginationArea>
        ...
        <NextPageBtn
          onClick={next}
          className={`${offset === totalPages ? "disabled" : ""}`}
        />
      </PaginationArea>
    </Container>
  );
